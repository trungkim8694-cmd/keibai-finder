...
### 11. Kiến Trúc Lưu Trữ Frontend - Tách Rời (Decoupled Storage Architecture)
- **Supabase Storage Trực Tiếp:** Toàn bộ hệ thống lưu trữ Hình ảnh (Images) và Hồ sơ (PDFs) đã được di dời hoàn toàn khỏi ổ cứng nội bộ của máy chủ AWS (Nginx) sang hệ thống Đám mây **Supabase Storage Bucket (`keibai-storage`)**. 
